...
- The user asks to create or refresh the host-root pointer, local project overlays, MCP profile, design-reference profile, verification facts, or path indexes.
...
5. Read existing `project/**` overlays when present.
6. Inspect only relevant manifests, configs, source entrypoints, routes, styles, assets, and verification scripts.
7. Read current `skills/*/agents/openai.yaml` files for MCP dependency scan.
...
5. If the project is outside the target stack, continue with limited onboarding: record the stack boundary, skip React/Next path indexes and stack-specific patterns, and write `project/skill-applicability-profile.md` with applicable framework-agnostic skills from `common/skill-applicability-policy.md`.
6. Scan `skills/*/agents/openai.yaml` for declared MCP dependencies and cache required, available, missing, optional, approved, installed, skipped, or blocked capabilities in `project/mcp-profile.md`.
7. In Plan Mode, return the plan and stop.
8. In approved execution mode, create or update only the approved pointer and local-only overlays, then run available validation checks.
