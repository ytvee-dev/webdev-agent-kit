...
- adjust small CSS spacing, wrapping, or compact heading/list styles;
...
- target one or two files by default;
- do not add packages;
- do not change API, WebSocket frames, stores, routing, auth, data flow, build tooling, or architecture layers;
...
For a micro UI fix or other lightweight code-changing task, verify in this order:
...
5. Rendered QA.
   - Run only when the user explicitly requests browser evidence, the route is accessible without new auth/session/test-data setup, and Browser or Playwright navigation tools are available.
